...
function sc_render_event_config_meta_box() {
	...
	echo '<table class="form-table">';
		...
		echo '<tr class="sc_meta_box_row">';
			...
			echo '<td class="sc_meta_box_td" colspan="2"><label for="sc_event_date">' . __('Event Start Date', 'pippin_sc') . '</label></td>';
		...
		echo '<tr>';
		echo '<td class="sc_meta_box_td" colspan="2"><label for="sc_event_end_date">' . __('Event End Date', 'pippin_sc') . '</label></td>';

		echo '<td class="sc_meta_box_td" colspan="4">';
				$enddate = isset( $meta['sc_event_end_date'][0] ) ? date( 'm/d/Y', $meta['sc_event_end_date'][0] ) : '';
				echo '<input type="text" class="sc_datepicker" name="sc_event_end_date" value="' . esc_attr( $enddate ) . '" placeholder="mm/dd/yyyy"/>';
			echo '</td>';
		echo '</tr>';
...
function sc_meta_box_save($post_id) {
	...
	if(empty($_POST['sc_event_date']) || !isset($_POST['sc_event_date']))
		return $post_id;

	$date 			= sanitize_text_field( $_POST['sc_event_date'] );
	if(!empty($_POST['sc_event_end_date']) && strtotime($_POST['sc_event_date']) < strtotime($_POST['sc_event_end_date']) ){
		$enddate        = sanitize_text_field( $_POST['sc_event_end_date'] );
	}else{
		$enddate        = sanitize_text_field( $date );
	}
	...
	update_post_meta($post_id, 'sc_event_end_date', strtotime($enddate));
