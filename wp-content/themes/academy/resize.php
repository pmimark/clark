...
/*
    ...
    wt: enable watermark
    gc: наложить черно-белый фильтр
    
    HTML example: <img src="/scripts/timthumb.php?src=/images/whatever.jpg&w=150&h=200&zc=1" alt="" />
	
	Дописал возможность кропалки через ЖС на Аяксе, просто в передаваемом параметре нада дописать flag_show которому присвоить значение 1 
*/
...
define ('WATERMARK', './watermark.png');    // path to watermark image
...
if (strlen ($src) && file_exists ($src)) {
    ...
    if(isset($_REQUEST['flag_show']) && $_REQUEST['flag_show']==1){
		my_show_img($mime_type,$canvas); 
	}else{
 ...
 *
 */
function my_show_img($mime_type,$canvas){
		global $quality;
		$path_to_cache = get_cache_file($mime_type);
		if (touch ($path_to_cache)) {

	        // give 666 permissions so that the developer
	        // can overwrite web server user
	        chmod ($path_to_cache, 0666);
	        $is_writable = 1;
		...
		if (stristr ($mime_type, 'jpeg')) {
			imagejpeg ($canvas, $path_to_cache, $quality);
		} else {
			$quality = floor ($quality * 0.09);
			imagepng ($canvas, $path_to_cache, $quality);
	    }
		
		echo substr_replace($path_to_cache,'',0,1);
}
...
function show_image ($mime_type, $image_resized) {
    ...
    if(get_request('wt',false)){
        $image_resized = waterMark($image_resized);
    }
    
    if(get_request('gc',false)){
...
function imgTOgray($canvas){
    for ($color = 0; $color <= 255; $color++) {
        imageColorAllocate($canvas, $color, $color, $color);
    }
...
function waterMark($canvas){
    ...
    $canvas_height = imagesx($canvas);
    
    $watermark = imageResize(WATERMARK,round($canvas_width/2));
    
    $wt_width = imagesx($watermark);
    $wt_height = imagesy($watermark);
    
    imagecopy($canvas, $watermark,round($canvas_width/4),20,0,0,$wt_width,$wt_height);
    
    return $canvas;
}

function imageResize($src = '', $width = 0, $height = 0)
{   
    ...
    if($width > 0 && $height > 0){
        $wRatio = $currentWidth/$width;
        $hRatio = $currentHeight/$height;
        
        $newHeight = round($currentHeight/$wRatio);
        $newWidth = round($currentWidth/$hRatio);
