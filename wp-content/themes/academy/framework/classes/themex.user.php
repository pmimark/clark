...
class ThemexUser {
	...
	public static function renderAdminProfile($user) {
		...
		if(current_user_can('edit_users')) {
			...
			$out.='<th><label for="avatar">'.__('Profile Photo', 'academy').'</label></th><td><div class="themex-image-uploader">';
	...
	public static function updateAdminProfile($user) {
		...
		if(isset($_POST['avatar']) && !empty($_POST['avatar']) && current_user_can('edit_users')) {
			$query="SELECT ID FROM ".$wpdb->posts." WHERE guid = '".esc_url($_POST['avatar'])."'";
			$ID=$wpdb->get_var($query);
			
			if(!empty($ID)) {
				ThemexCore::updateUserMeta($user, 'avatar', $ID);
			}
		}
	}
