...
class ThemexLesson {
	...
	public static function init() {
		...
		//redirect lesson
		add_action('template_redirect', array(__CLASS__,'redirectLesson'), 99);
     ...
     * @return void
     */
	public static function redirectLesson() {
		global $post;
		$type=get_post_type();
		...
		if(in_array($type, array('lesson', 'quiz'))) {
			...
			if((!ThemexCourse::isSubscriber() || !ThemexCourse::isMember()) && !ThemexCourse::isAuthor() && $status!='free') {
				$redirect=SITE_URL;
				if($course!=0) {
