...
class ThemexCourse {
	...
	public static function getCourse($ID, $extended=false) {
		...
		if($extended) {
			...
			$course['lessons']=self::getLessons($ID);
	...
	public static function isAuthor() {
		if(self::$data['author']['ID']==get_current_user_id() || current_user_can('manage_options')) {
			return true;
		}
	...
	public static function isMember($user=0) {
        ...
        if(empty(self::$data['users']))
            self::$data['users'] = array();
